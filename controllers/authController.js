...
async function login(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: 'Logged in successfully.', token, user: safeUser });
  } catch (err) {
...
async function createAdmin(req, res) {
  ...
  try {
    ...
    return res.status(201).json({
      ...
      user: { id: rows[0].id, name, email, role: 'admin', location: location || null },
    });
  } catch (err) {
...
async function updateProfile(req, res) {
  try {
    ...
    const values = [name, email, location || null, bio || null];
