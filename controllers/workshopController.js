...
// GET /api/workshops/my (must be registered BEFORE /:id in routes)
...
async function deleteWorkshop(req, res) {
  try {
    const [, meta] = await pool.query('DELETE FROM workshops WHERE id = ?', [req.params.id]);
...
async function enrollInWorkshop(req, res) {
  try {
    ...
    if (parseInt(ws[0].enrolled_count) >= ws[0].capacity)
